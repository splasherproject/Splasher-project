...
express.get("/fortnite/api/cloudstorage/user/*/:file", async (req, res) => {
    try {
        if (!fs.existsSync(path.join(process.env.LOCALAPPDATA, "Splasher", "ClientSettings"))) {
            fs.mkdirSync(path.join(process.env.LOCALAPPDATA, "Splasher", "ClientSettings"));
        }
    } catch (err) {}
    ...
    let file;
    if (process.env.LOCALAPPDATA) file = path.join(process.env.LOCALAPPDATA, "Splasher", "ClientSettings", `ClientSettings-${currentBuildID}.Sav`);
